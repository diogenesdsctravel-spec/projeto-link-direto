...
export function VooVolta({ cotacaoData, onNext, onPrev }: ScreenProps) {
    const { snapshot } = cotacaoData;

    // Extrai dados do voo de volta
    const voo = getVoo(snapshot, "volta");

    // Se não houver voo, mostra mensagem
    if (!voo) {
        return (
            <div className="h-full flex items-center justify-center bg-white">
                <p className="text-gray-500">Voo de volta não incluído neste pacote</p>
            </div>
        );
    }
    ...
    // Dados do destino da viagem (para textos emocionais)
    const destinoViagem = snapshot.meta?.destino_nome || "seu destino";

    // Formata data para exibição (ex: "06/04 • Domingo")
            ...
            <div data-scrollable="true" className="flex-1 overflow-y-auto -mt-6 relative z-10">
                <div className="bg-white rounded-t-3xl px-6 pt-8 pb-32">
                    <h1 className="text-[#09077d] text-3xl mb-3 leading-tight">
                        Voltando transformado<br />
                        {diaSemana ? `no ${diaSemana.toLowerCase()}` : "para casa"}
                    </h1>
                    ...
                    <div className="space-y-5 mb-8 text-gray-700 leading-relaxed">
                        <p>
                            {diaSemana && (
                                <>
                                    No <span className="text-[#09077d] font-medium">{diaSemana}, {data}</span>,
                                </>
                            )}{' '}
                            após dias que renovaram sua alma, você decola às{' '}
                            <span className="text-[#09077d] font-medium">{horarioSaida}</span> de {origemCidade}.
                        </p>

                        <p>
                            Após <span className="text-[#09077d] font-medium">{duracao} de voo tranquilo</span>,
                            você pousa em {destinoCidade} às <span className="text-[#09077d] font-medium">{horarioChegada}</span>,
                            levando {destinoViagem} no coração.
                        </p>

                        <p className="text-sm text-gray-600 italic">
                            Você volta diferente — mais leve, conectado e renovado.
                        </p>
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white via-white to-transparent pt-8 pb-8 px-6 z-20">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-[#09077d] text-sm text-center">
                        Deslize para ver o resumo completo
                    </p>
                </div>
